...
#import "Msg_CommentsCell.h"


@interface Msg_CommentsVC ()<UITableViewDataSource, UITableViewDelegate,FriendCircleDelegate>
@property (nonatomic, strong)UITableView *tableView;
@property (nonatomic, strong)NSMutableArray *dataSource;
@property (nonatomic, strong)UIButton *rightbBarButton;
@end

@implementation Msg_CommentsVC
...
- (void)requestData{
    NSString *urlString = [NSString stringWithFormat:URL_FC_UnreadComments_List,User_Token,self.pageNumber,Page_Count];
    DDWeakSelf;
...
- (void)deleteMsg:(NSInteger)index {
    ...
    self.rightbBarButton.enabled = NO;
    DDWeakSelf;
    [ClassTool postRequest:URL_DeleteMSG_Comments Params:[dict mutableCopy] Success:^(id json) {
        if ([json[@"code"] isEqualToString:@"SUCCESS"]) {
            weakself.rightbBarButton.enabled = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    Msg_CommentsCell *cell = [Msg_CommentsCell cellWithTableView:tableView];
