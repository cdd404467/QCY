...
@interface Msg_RemindUsersVC ()<UITableViewDataSource, UITableViewDelegate,FriendCircleDelegate>
@property (nonatomic, strong)UITableView *tableView;
@property (nonatomic, strong)NSMutableArray *dataSource;
@property (nonatomic, strong)UIButton *rightbBarButton;
@end

@implementation Msg_RemindUsersVC
...
- (void)requestData{
    NSString *urlString = [NSString stringWithFormat:URL_FC_AtMe_List,User_Token,self.pageNumber,Page_Count];
    DDWeakSelf;
...
- (void)deleteMsg:(NSInteger)index {
    ...
    self.rightbBarButton.enabled = NO;
    DDWeakSelf;
    [ClassTool postRequest:URL_DeleteMSG_AtMe Params:[dict mutableCopy] Success:^(id json) {
        if ([json[@"code"] isEqualToString:@"SUCCESS"]) {
            weakself.rightbBarButton.enabled = YES;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FCMsgModel *model = _dataSource[indexPath.row];
    vc.tieziID = model.communityId;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    Msg_RemindUsersCell *cell = [Msg_RemindUsersCell cellWithTableView:tableView];
