...
- (void)setupBottomView {
    ...
    [zanBtn setTitleColor:HEXColor(@"#3C3C3C", .5) forState:UIControlStateDisabled];
    ...
    [zanBtn setTitle:@"已赞" forState:UIControlStateDisabled];
...
- (void)requestData {
    ...
    NSString *urlString = [NSString stringWithFormat:URL_FC_Detail,User_Token,_tieziID];
    if (_messageType)
        urlString = [urlString stringByAppendingString:[NSString stringWithFormat:@"&messageType=%@&messageId=%@",_messageType,_messageId]];
...
//        NSLog(@"---- %@",json);
        if ([To_String(json[@"code"]) isEqualToString:@"SUCCESS"]) {
            ...
            weakself.zanBtn.enabled = [weakself.headerDataSource.isLike integerValue] == 1 ? NO : YES;
...
- (void)setupPageVC {
    ...
    header.ziXunClickBlock = ^(NSString * _Nonnull zixunID) {
...
- (void)setupUI {
    ...
    //咨询view
    _ziXunView = [[FCZiXunView alloc] initWithFrame:CGRectMake(headerImage.left + 10, 0, SCREEN_WIDTH - (headerImage.left + 10) * 2 , 50)];
    _ziXunView.backgroundColor = HEXColor(@"#f3f3f3", 1);
    [HelperTool addTapGesture:_ziXunView withTarget:self andSEL:@selector(zixunClick)];
    [self addSubview:_ziXunView];
}
...
- (void)setHeaderModel:(FriendCricleModel *)headerModel {
    ...
    if isRightData(headerModel.postUserPhoto) {
        if ([[headerModel.postUserPhoto substringToIndex:4] isEqualToString:@"http"]) {
            [_headerImage sd_setImageWithURL:[NSURL URLWithString:headerModel.postUserPhoto] placeholderImage:PlaceHolderImg];
        } else {
    ...
    //咨询View
    if (isRightData(headerModel.shareBean.title)) {
        _ziXunView.model = headerModel.shareBean;
        _ziXunView.top = _bottom;
        _ziXunView.hidden = NO;

        _bottom = _ziXunView.bottom + 20;
    } else {
...
- (void)zixunClick {
    if (self.ziXunClickBlock) {
        self.ziXunClickBlock(_headerModel.shareBean.zixunID);
    }
}
