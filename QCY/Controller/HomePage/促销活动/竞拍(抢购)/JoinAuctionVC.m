...
- (void)setupUIWithInfoDict:(NSDictionary *)dict {
    ...
    NSString *text = @"购买提交后，1个工作日内，平台将会电话确认，确认最终交易细节，引导双方签署协议履行合同！";
    ...
    //for循环创建
    NSArray *titleArr = @[@"我的报价:",@"是否需要样品:",@"联系人:",@"联系人方式:", @"公司名称:",@"公司所属区域:",@"公司详细地址:"];
    ...
    priceTipLab.text = [NSString stringWithFormat:@"当前最高价：%@%@，价差：%@",_nowPrice,_priceUnit,_addPrice];
...
-  (BOOL)judgeRight {
    ...
    if (price.length == 0) {
        [CddHUD showTextOnlyDelay:@"请输入你的报价" view:self.view];
        return NO;
    } else if (_jpCount == 0 && [price doubleValue] < [_startPrice doubleValue]) {
        [CddHUD showTextOnlyDelay:@"报价不能小于最低价" view:self.view];
        return NO;
    } else if (_jpCount != 0 && [price doubleValue] < [_nowPrice doubleValue]) {
        [CddHUD showTextOnlyDelay:@"报价不能小于当前最高价" view:self.view];
        return NO;
    } else if ([self judgeFloat:([price doubleValue] - [_nowPrice doubleValue]) / [_addPrice doubleValue]] == NO) {
        [CddHUD showTextOnlyDelay:[NSString stringWithFormat:@"报价必须为%@的整数倍",_addPrice] view:self.view];
        return NO;
