...
@interface OrderGoodsVC ()<UITableViewDelegate, UITableViewDataSource>
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MobClick beginLogPageView:[NSString stringWithFormat:@"采购联盟-采购-%@",_pName]];
}

- (void)viewWillDisappear:(BOOL)animated
{
    [super viewWillDisappear:animated];
    [MobClick endLogPageView:[NSString stringWithFormat:@"采购联盟-采购-%@",_pName]];
}
    ...
    for (MeetingShopListModel *model in _dataSource.meetingList.data) {
        if (model.isSelect == YES) {
            ...
            NSDictionary *goodsDict = @{@"shopName":model.shopName,
                                        ...
                                        @"diyShop ":boolStr,
                                        @"numUnit":@"KG",
                                        @"reservationNum":model.reservationNum,
                                        @"orderStatus":@"0",
            ...
            [goodsArr addObject:[goodsMutablDict copy]];
        }
    }
...
//        NSLog(@"-----== %@",json);
        if ([To_String(json[@"code"]) isEqualToString:@"SUCCESS"]) {
            [CddHUD showTextOnlyDelay:@"订货成功" view:weakself.view];
...
- (void)requestData {
    DDWeakSelf;
    NSString *urlString = [NSString stringWithFormat:URL_System_Goods_List,_goodsID,@"0"];
...
//        ContactsInfoCell *cell = [ContactsInfoCell cellWithTableView:tableView];
        ...
        [cell.cellImageView sd_setImageWithURL:ImgUrl(_dataSource.picIn) placeholderImage:PlaceHolderImg completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            /** 缓存image size */
