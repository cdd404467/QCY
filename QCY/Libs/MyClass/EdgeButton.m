...
- (void)p_layoutSubViewsForImagePositionLeft {
    if (self.contentHorizontalAlignment == UIControlContentHorizontalAlignmentRight) {   // 整体靠右
        ...
        self.titleLabel.left = self.width - self.titleLabel.width;
        self.titleLabel.top = (self.height - self.titleLabel.height) * 0.5;
        
        self.imageView.left = self.width - self.titleLabel.width - self.interTitleImageSpacing - self.imageView.width;
        self.imageView.top = (self.height - self.imageView.height) * 0.5;
        
    } else if (self.contentHorizontalAlignment == UIControlContentHorizontalAlignmentLeft) { // 整体靠左
        self.imageView.left = 0;
        self.imageView.top = (self.height - self.imageView.height) * 0.5;
        
        self.titleLabel.left = self.imageView.right + self.interTitleImageSpacing;
        self.titleLabel.top = (self.height - self.titleLabel.height) * 0.5;
        
    } else if (self.contentHorizontalAlignment == UIControlContentHorizontalAlignmentCenter) { // 整体居中
        self.imageView.left = self.width * 0.5 - (self.titleLabel.width + self.interTitleImageSpacing + self.imageView.width) * 0.5;
        self.imageView.top = (self.height - self.imageView.height) * 0.5;
        
        self.titleLabel.left = self.interTitleImageSpacing + self.imageView.right;
        self.titleLabel.top = (self.height - self.titleLabel.height) * 0.5;
    }
}
...
- (void)p_layoutSubViewsForImagePositionRight {
    if (self.contentHorizontalAlignment == UIControlContentHorizontalAlignmentRight) {   // 整体靠右
        ...
        self.imageView.left = self.width - self.imageView.width;
        self.imageView.top = (self.height - self.imageView.height) * 0.5;
        
        self.titleLabel.left = self.width - self.imageView.width - self.interTitleImageSpacing - self.titleLabel.width;
        self.titleLabel.top = (self.height - self.titleLabel.height) * 0.5;
        ...
        self.imageView.left = self.interTitleImageSpacing + self.titleLabel.width;
        self.imageView.top = (self.height - self.imageView.height) * 0.5;
        
    } else if (self.contentHorizontalAlignment == UIControlContentHorizontalAlignmentCenter) { // 整体居中
        self.titleLabel.left = self.width * 0.5 - (self.titleLabel.width + self.interTitleImageSpacing + self.imageView.width) * 0.5;
        self.titleLabel.top = (self.height - self.titleLabel.height) * 0.5;
        
        self.imageView.left = self.titleLabel.left + self.titleLabel.width + self.interTitleImageSpacing;
        self.imageView.top = (self.height - self.imageView.height) * 0.5;
    }
}
...
- (void)p_layoutSubViewsForImagePositionTop {
    if (self.contentVerticalAlignment == UIControlContentVerticalAlignmentTop) {  // 整体靠顶部
        ...
        self.titleLabel.top = self.imageView.bottom + self.interTitleImageSpacing;
        ...
        self.imageView.top = self.height - (self.imageView.height + self.titleLabel.height + self.interTitleImageSpacing);
        self.imageView.centerX = self.width * 0.5;
        
    } else if (self.contentVerticalAlignment == UIControlContentVerticalAlignmentCenter) { // 整体居中
        self.imageView.top = self.height * 0.5 - (self.imageView.height + self.titleLabel.height + self.interTitleImageSpacing) * 0.5;
        self.imageView.centerX = self.width * 0.5;
...
- (void)p_layoutSubViewsForImagePositionBottom {
    if (self.contentVerticalAlignment == UIControlContentVerticalAlignmentTop) {  // 整体靠顶部
        ...
        self.imageView.top = self.titleLabel.bottom + self.interTitleImageSpacing;
        self.imageView.centerX = self.width * 0.5;
        
    } else if (self.contentVerticalAlignment == UIControlContentVerticalAlignmentBottom) { // 整体靠底部
        
        self.imageView.top = self.height - self.imageView.height;
        self.imageView.centerX = self.width * 0.5;
        
        self.titleLabel.top = self.height - (self.titleLabel.height + self.interTitleImageSpacing + self.imageView.height);
        self.titleLabel.centerX = self.width * 0.5;
        
    } else if (self.contentVerticalAlignment == UIControlContentVerticalAlignmentCenter) { // 整体居中
        
        self.titleLabel.top = self.height * 0.5 - (self.imageView.height + self.titleLabel.height + self.interTitleImageSpacing) * 0.5;
        self.titleLabel.centerX = self.width * 0.5;
