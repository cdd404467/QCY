...
@property (nonatomic, strong) UIButton *ztcBtn;
@end
...
//    if (!GET_USER_TOKEN) {
        ...
        //求购直通车
        UIButton *ztcBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [ztcBtn setTitle:@"求购直通车" forState:UIControlStateNormal];
        ztcBtn.titleLabel.font = [UIFont systemFontOfSize:11];
        ztcBtn.userInteractionEnabled = NO;
        [ztcBtn setTitleColor:UIColor.whiteColor forState:UIControlStateNormal];
        ztcBtn.backgroundColor = HEXColor(@"#FF771C", 1);
        [ztcBtn layoutWithEdgeInsetsStyle:ButtonEdgeInsetsStyleLeft imageTitleSpace:3.f];
        [ztcBtn setImage:[UIImage imageNamed:@"ztc_icon"] forState:UIControlStateNormal];
        [bgView addSubview:ztcBtn];
        [ztcBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.height.mas_equalTo(userType);
            make.left.mas_equalTo(userType.mas_right).offset(10);
            make.width.mas_equalTo(KFit_W(86));
        }];
        _ztcBtn = ztcBtn;
        [ztcBtn.superview layoutIfNeeded];
        [HelperTool setRound:ztcBtn corner:UIRectCornerBottomLeft | UIRectCornerBottomRight radiu:5.f];
        ...
        [personImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.mas_equalTo(ztcBtn.mas_right).offset(KFit_W(10));
...
//倒计时和公司名字
- (void)configData:(AskToBuyModel *)model {
    //剩余时间
...
//下半部分
- (void)configBottomData:(AskToBuyModel *)model {
    //企业还是个人图片
    ...
    if (model.showInfo.integerValue == 1) {
        _ztcBtn.hidden = NO;
        [_personImageView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self.ztcBtn.mas_right).offset(10);
            make.top.mas_equalTo(0);
            make.width.height.mas_equalTo(self.userType);
        }];
    } else {
        _ztcBtn.hidden = YES;
        [_personImageView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self.userType.mas_right).offset(10);
            make.top.mas_equalTo(0);
            make.width.height.mas_equalTo(self.userType);
