...
- (void)setupUI:(NSInteger)number model:(OpenMallModel *)model {
    ...
    CGFloat nameMaxWidth = SCREEN_WIDTH - 185 - 25 - 5;
    CGFloat maxWidth = [model.companyName boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 15)
    ...
    UIButton *ghBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [ghBtn setImage:[UIImage imageNamed:@"gh_bg"] forState:UIControlStateNormal];
    [bgView addSubview:ghBtn];
    [ghBtn addTarget:self action:@selector(jumpToCheck) forControlEvents:UIControlEventTouchUpInside];
    [ghBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(companyName.mas_right).offset(6);
...
- (void)jumpToCheck {
    if (self.jumpToCheckBlock) {
        self.jumpToCheckBlock();
    }
}
...
- (void)callPhone {
    ...
    } else if (isRightData(_model.company.tel)) {
        phoneNum = _model.company.tel;
    } else {
