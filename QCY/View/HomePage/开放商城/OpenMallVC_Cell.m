...
- (void)setModel:(OpenMallModel *)model {
    ...
    if (isRightData(model.phone)) {
        phone = [NSString stringWithFormat:@"联系方式：%@",model.phone];
    } else if (isRightData(model.company.tel)) {
        phone = [NSString stringWithFormat:@"联系方式：%@",model.company.tel];
    } else {
        phone = @"联系方式：暂无";
    }
    _contactLabel.text = phone;
