...
- (void)setupUI {
    ...
    //产品名字
    _nameLab = [[UILabel alloc] initWithFrame:CGRectMake(11, _nowPriceView.bottom + 10, SCREEN_WIDTH - 11 * 2, 15)];
...
- (void)setModel:(AuctionModel *)model {
    ...
    //竞拍次数
    NSMutableAttributedString *mutableCount = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@次出价",model.count]];
    ...
    if ([model.isType isEqualToString:@"0"]) {
        NSMutableAttributedString *mutableNowPrice = [[NSMutableAttributedString alloc] initWithString:@"拍卖已流拍，\n无人参拍！"];
    ...
    _addPriceLab.text = [NSString stringWithFormat:@"加价幅度: ¥%@",model.addPrice];
    
    //运费
    _freightLab.top = _addPriceLab.top;
    _freightLab.text = [NSString stringWithFormat:@"运费: %@",model.freight];
    ...
    //生产日期
    _producedDateLab.top = _producedCJLab.top;
    ...
    _viewHeight = _areaLab.bottom + 10;
    //自定义
    if (model.auctionDetails.length != 0 && model.auctionDetails1.length != 0) {
        _customLab_1.top = _areaLab.bottom + 5;
        ...
        _customLab_2.top = _areaLab.bottom + 5;
        _customLab_2.text = [NSString stringWithFormat:@"%@: %@",model.auctionDetails1,model.detailsValue1];
