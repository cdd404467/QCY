...
@implementation Msg_RemindUsersCell
...
- (void)setupUI {
    ...
    //@ 我
    NSString *str = @"@";
    CGFloat width = [str boundingRectWithSize:CGSizeMake(50, 15)
                                      options:NSStringDrawingUsesLineFragmentOrigin
                                   attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:15]}
                                      context:nil].size.width + 2;
    //赞了我
    UILabel *atLab= [[UILabel alloc] init];
    atLab.text = str;
    atLab.textColor = HEXColor(@"#ED3851", 1);
    atLab.font = [UIFont boldSystemFontOfSize:15];
    [self.contentView addSubview:atLab];
    [atLab mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [zanLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(atLab.mas_right).offset(4);
        make.right.height.mas_equalTo(certNameLabel);
        make.centerY.mas_equalTo(atLab.mas_centerY);
    }];
...
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    static NSString *identifier = @"Msg_RemindUsersCell";
    Msg_RemindUsersCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[Msg_RemindUsersCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
