...
- (void)setupUI {
    ...
    for (NSInteger i = 0; i < 9; i++) {
        ...
        playImg.image = [UIImage imageNamed:@"YBImageBrowserVideo.bundle/ybib_bigPlay"];
        playImg.frame = CGRectMake(0, 0, 60, 60);
        playImg.hidden = YES;
...
- (void)tap:(UIGestureRecognizer *)gesture
{
    ...
    [self.urlArray enumerateObjectsUsingBlock:^(NSString *_Nonnull urlStr, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([self.type isEqualToString:@"photo"]) {
            ...
            data.projectiveView = weakself.imageViewsArray[idx];
            [browserDataArr addObject:data];
        } else {
            YBIBVideoData *data = [[YBIBVideoData alloc] init];
            data.videoURL = [NSURL URLWithString:weakself.videoURL];
            data.projectiveView = weakself.imageViewsArray[idx];
            [browserDataArr addObject:data];
        }
