...
//    _signLabel = signLabel;
    ...
    [countLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(productName.mas_bottom).offset(10);
    }];
...
- (void)configData:(MyAskToBuyModel *)model {
...
- (void)setModel:(MyAskToBuyModel *)model {
    _model = model;
    ...
    //报价时间
    _offertTime.text = [TimeAbout timestampToString:model.createdAt];

    //地区
    _areaLabel.text = [NSString stringWithFormat:@"地区: %@ %@",model.locationProvince,model.locationCity];
    //包装要求
    _requireLabel.text = [NSString stringWithFormat:@"包装要求: %@",model.pack];
//    //是否包含运费
...
//
    ...
    if isRightData(model.descriptionStr) {
        _bakLabel.text = model.descriptionStr;
    } else {
        _bakLabel.text = @"暂无";
    }
...
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    static NSString *identifier = @"MyAskToBuyCell";
    MyAskToBuyCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[MyAskToBuyCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
